...
export default defineContentScript({
  ...
  async main() {
    ...
    function createAIIcon(targetElement: HTMLElement) {
      // Find the parent element of the focused element
      ...
      aiIcon.id = "ai-icon";
      // Add your exported SVG here (from Figma)
      aiIcon.innerHTML = `<svg width="14" height="12" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
      ...
      // Add Tailwind classes for positioning and styling
      aiIcon.style.position = "absolute";
      aiIcon.style.right = "0.5rem";
      aiIcon.style.bottom = "4px";
      aiIcon.style.cursor = "pointer";
      aiIcon.style.zIndex = "9999";
      aiIcon.style.borderRadius = "50%";
      aiIcon.style.backgroundColor = "#fff";
      aiIcon.style.border = "1px solid #e5e7eb";
      aiIcon.style.padding = "0.5rem";

      // Append the AI icon to the parent div of the focused input
      parentDiv.appendChild(aiIcon);

      // Add an event listener to handle modal opening when the icon is clicked
    ...
    function insertResponse(responseText: string) {
      ...
      if (inputElement) {
        // Insert the response text as HTML
        inputElement.innerHTML = `<p>${responseText}</p>`;
    ...
    function handleFocusIn(event: Event) {
      ...
      if (target.matches(inputSelector)) {
        if (!aiIcon) createAIIcon(event.target as HTMLElement);
      }
    }
